...
std::vector<ProcessInfo> GetRobloxProcesses()
{
	...
	for (HANDLE handle : GetProcessesByImageName("ShklurbPlayerBeta.exe")) result.emplace_back(ProcessInfo(handle, "ShklurbPlayerBeta.exe"));
...
DWORD WINAPI WatchThread(LPVOID)
{
	...
	while (1)
	{
		auto processes = GetProcessesByImageName("ShklurbPlayerBeta.exe"); // no studio for now
